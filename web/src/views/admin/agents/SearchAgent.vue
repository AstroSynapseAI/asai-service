...
const toggleSearchEngine = (id) => {
  ...
  if (id === 'ddg') {
    config.ddg_is_active = !config.ddg_is_active
  }
  
  if (id === 'google') {
    config.google_is_active = !config.google_is_active
  }

  if (id === 'exa') {
    config.exa_is_active = !config.exa_is_active
  }
}
            ...
            <div class="row">
              <div clas="col-12">
                ...
                <div class="form-check form-switch">
                  <input class="form-check-input" type="checkbox" id="ddg-checkbox" @click="toggleSearchEngine('ddg')" :checked="config.ddg_is_active">
                </div>

                <span> Google Search</span>
                <div class="form-floating mb-3">
                  <input type="text" class="form-control" id="apiToken" placeholder="Tool Token" v-model="config.google_api_token">
                  <label for="apiToken">Api Token</label>
                </div>
                <div class="form-check form-switch">
                  <input class="form-check-input" type="checkbox" id="google-checkbox" @click="toggleSearchEngine('google')" :checked="config.google_is_active">
                </div>

                <span> Exa Search</span>
                
                <div class="form-floating mb-3">
                  <input type="text" class="form-control" id="apiToken" placeholder="Tool Token" v-model="config.exa_api_token">
                  <label for="apiToken">Api Token</label>
                </div>
                <div class="form-check form-switch">
                  <input class="form-check-input" type="checkbox" id="exa-checkbox" @click="toggleSearchEngine('exa')" :checked="config.exa_is_active">
                </div>
              </div>
            </div>
